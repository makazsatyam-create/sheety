...
function MarketSection({
  ...
  const isThisMarketSelected =
    ...
    selectedBet?.marketRowIndex !== null
      ...
      : selectedBet?.marketTitle === title && selectedBet?.odd != null;
          ...
          <TableBody>
            {data.map((row, i) => {
              ...
              return (
                ...
                {/* Mobile: bet slip directly below the selected row */}
...
function getGroupedFancySections(battingData) {
  ...
  battingData
    ...
    .forEach((market) => {
      ...
      seenIds.add(id);
      sections.push({ id, label, rows: market.section.map(sectionToRow) });
    });
  return sections.filter((s) => (s.rows || []).length > 0);
}

const defaultMatchInfo = { teams: "Football Match", date: "", time: "" };

export default function PreviewPage1Football() {
  ...
  const match =
    location.state?.match ?? useSelector((state) => state.soccer.selectedMatch);
  ...
  useEffect(() => {
    ...
    dispatch(fetchSoccerBatingData(gameIdFromUrl));
  }, [gameIdFromUrl, dispatch]);

  useEffect(() => {
    const gid = gameIdFromUrl || match?.id;
    if (!gid) return;
    wsService.connect(dispatch, userId);
    wsService.subscribe(gid, "soccer", null);
  }, [gameIdFromUrl, match?.id, dispatch, userId]);

  useEffect(() => {
    if (gameId) dispatch(getPendingBet(gameId));
  }, [gameId, dispatch]);

  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("multiMarkets") || "[]");
    setHasMultiMarkets(saved.length > 0);
  }, []);

  // Redirect if all market sections are empty (same logic as Soccerbet)
  useEffect(() => {
    if (
    ...
    const allSectionsEmpty =
      marketSectionMarkets.length === 0 && groupedFancySections.length === 0;
    if (allSectionsEmpty) {
    ...
    battingData,
    marketSectionMarkets.length,
    groupedFancySections.length,
    navigate,
  ]);
  ...
  const handlePlaceBet = async () => {
    ...
    const teamName = selectedBet.team || "";
    ...
    try {
      await dispatch(createBet(formData)).unwrap();
      setSelectedBet({
        team: "",
        odd: "",
        side: "",
    ...
    <>
      ...
      >
        ...
        >
            ...
            {marketSectionMarkets.map((market, rowIndex) => {
              ...
              const isSelectedFromThisMarket =
                ...
                const mobileSlipContent = isSelectedFromThisMarket ? (
                  <Box sx={{ display: { xs: "block", md: "none" }, mt: 1, px: 1, pb: 1 }}>
          ...
          {/* Right Sidebar - sticky; no overflow:hidden on ancestors */}
          <Box
            sx={{
