...
export const updateCreditReference = async (req, res) => {
  try {
    ...
    console.log('🚨 [CREDIT REF UPDATE] User:', editUser.userName);
    console.log('🚨 [CREDIT REF UPDATE] Base Balance:', editUser.baseBalance);
    console.log('🚨 [CREDIT REF UPDATE] Credit Reference:', creditReference);
    console.log(
      '🚨 [CREDIT REF UPDATE] Credit Ref P/L:',
      editUser.creditReferenceProfitLoss
    );
...
export const updateExploserLimit = async (req, res) => {
  try {
    ...
    // ✅ Save updated user
    console.log(
      `🚨 [USER-SAVE] subAdminController saving user ${editUser.userName}`
    );
    await editUser.save();
    const filter =
      role === 'supperadmin'
