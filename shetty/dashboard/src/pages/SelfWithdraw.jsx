...
const SelfWithdraw = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const response = await api.get('/withdrawal-requests', { params: {} });
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      const response = await api.put(`/withdrawal-requests/${requestId}/approve`);
      if (response.data.success) {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await api.put(`/withdrawal-requests/${selectedRequest._id}/reject`, {
        rejectionReason,
      });
      if (response.data.success) {
  ...
  return (
    <>
      ...
      <div className="px-3.5 md:px-7.5 py-5">
        <div className="rounded-sm border border-gray-300 bg-white p-5">
          <h1 className="mb-6 text-2xl font-bold text-gray-800">Self Withdrawal Requests</h1>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-gray-500">
              No {filterStatus === 'all' ? '' : filterStatus} withdrawal requests found
            </div>
          ) : (
          ...
          <div className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <>
                    {selectedRequest.accountNumber && (
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.accountNumber}</p>
                      </div>
                    )}
                    {selectedRequest.accountHolderName && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Account Holder Name</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.accountHolderName}</p>
                      </div>
                    )}
                    {selectedRequest.bankName && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Bank Name</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.bankName}</p>
                      </div>
                    )}
                    {selectedRequest.branchName && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Branch Name</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.branchName}</p>
                      </div>
                    )}
                    {selectedRequest.ifscCode && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">IFSC Code</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.ifscCode}</p>
                      </div>
                    )}
                  </>
                )}
                ...
                {/* Crypto Details */}
                {selectedRequest.paymentType === 'crypto' && (
                  <>
                    {selectedRequest.currency && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Currency</label>
                        <p className="mt-1 text-sm text-gray-900">{selectedRequest.currency}</p>
                      </div>
                    )}
                    {selectedRequest.walletAddress && (
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-gray-700">Wallet Address</label>
                        <p className="mt-1 break-all text-sm text-gray-900">{selectedRequest.walletAddress}</p>
                      </div>
                    )}
                  </>
                )}
      ...
      {showRejectModal && selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h2 className="mb-4 text-xl font-bold text-gray-800">Reject Withdrawal Request</h2>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700">Rejection Reason</label>
              <textarea
